...
const Trace = () => {
  ...
  const init = () => {
    ...
    cx.fillStyle = 'rgb(200, 200, 250)'
    drawletter('5')
    cx.fillStyle = 'rgb(200, 200, 251)'
    letterpixels = getpixelamount(200, 200, 250)
  }
  const pixelthreshold = () => {
    if (getpixelamount(200, 200, 251) / letterpixels > 0.8) {
      cx.fillStyle = 'rgb(144,238,144)'
      drawletter('5')
      // alert('nice!')
    }
  }
  ...
  const paint = (x, y) => {
    ...
    if (colour.a === 0) {
      ...
      while (stack.length > 0) {
        ...
        if (pixel[0] < 0 || pixel[0] >= c.width) continue
        if (pixel[1] < 0 || pixel[1] >= c.height) continue
        if (((x - pixel[0]) ** 2 + (y - pixel[1]) ** 2) ** 0.5 > fillRange)
          continue
        const color = getpixelcolour(...pixel)
        if (color.a === 0) continue
        if (color.r === 200 && color.g === 200 && color.b === 251) continue

        cx.fillRect(pixel[0], pixel[1], 1, 1)
  ...
  const onmousemove = e => {
    const x = Math.round(e.clientX - c.getBoundingClientRect().left)
    const y = Math.round(e.clientY - c.getBoundingClientRect().top)
    if (touchdown) {
      paint(x, y)
    }
  }
  ...
  useEffect(() => {
    setC(canvasRef.current)
    console.log(c)
    setCx(c.getContext('2d'))
